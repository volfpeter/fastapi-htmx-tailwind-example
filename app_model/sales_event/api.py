...
from .model import SalesEvent


def make_api(*, get_db: DatabaseProvider, jinja: Jinja, prefix: str = "/sales-event") -> APIRouter:
    """
    Creates a new API router for sales events.
    ...
    template_prefix = "app_model/sales_event"

    table_row_template: Template = jinja.templates.get_template(
        f"{template_prefix}/sales-event-table-row.jinja"
    )
    ...
    @api.get("/{device_id}")
    @jinja.hx(f"{template_prefix}/sales-events-of-device-dialog.jinja", no_data=True)
    async def sales_events_of_device_dialog(
    ...
    async def stream_by_device_id(
        ...
        def encode_event(event: SalesEvent) -> str:
        ...
        return EventSourceResponse(
            event_source(
                request,
                device,
                make_event=SalesEvent.from_device,
